...
def check_for_obstacles(control):
	...
	executor.add_node(check_for_obstacles_client)

	my_name = control["my_name"]
	next_position = control["next_position"]
	print("Verificando obstáculos em ", next_position)
	future_request = check_for_obstacles_client.send_request(my_name, next_position)
	#executor.spin_until_future_complete(check_for_obstacles_client, future_request)
	executor.spin() #_once_until_future_complete(future_request)
	request_response = future_request.result()
	...
	control["obstacles_found"] = request_response.response
...
def inform_position(control):
	...
	executor.add_node(inform_position_client)
